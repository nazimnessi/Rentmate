...
class Request(models.Model):
    ...
    room = models.ForeignKey(
        "Room", on_delete=models.CASCADE, related_name="sent_requests"
    )
...
class Room(models.Model):
    ...
    rent_payment_date = models.DateField(
        "rent payment date",
        blank=True,
        null=True,
        help_text=("Date from which the payment of a month start"),
    )
    rent_payment_interval = models.IntegerField(
        blank=True,
        null=True,
    ...
    def clean(self):
        if self.renter == self.building.owner:
            raise ValidationError(
                "Owner of the building cannot be the renter of a room."
            )
...
class Utility(models.Model):
    ...
    latest_amount = models.DecimalField(
...
class Lease(models.Model):
    ...
    rent_payment_day = models.CharField(
        max_length=2,
        blank=True,
        null=True,
        help_text=("Date from which the payment of a month start"),
    ...
    def save(self, *args, **kwargs):
        ...
        if self.rent_period_start and self.rent_period_end:
            if current_datetime < datetime.combine(
                self.rent_period_start, datetime.min.time()
            ):
                self.status = "Upcoming"
            elif (
                datetime.combine(self.rent_period_end, datetime.min.time())
                < current_datetime
            ):
                self.status = "Expired"
            elif current_datetime > datetime.combine(
                self.rent_period_end, datetime.min.time()
            ) - timedelta(days=90):
                self.status = "Expiring Soon"
            elif (
                datetime.combine(self.rent_period_start, datetime.min.time())
                < current_datetime
                < datetime.combine(self.rent_period_end, datetime.min.time())
            ):
                self.status = "Current"
