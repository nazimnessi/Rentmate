...
class Query(graphene.ObjectType):
    all_Buildings = DjangoFilterConnectionField(
        BuildingType,
        orderBy=graphene.String() or None,
        globalSearch=graphene.Boolean(),
        searchTerm=graphene.String(),
    )
    Buildings = relay.Node.Field(BuildingType)

    all_Rooms = DjangoFilterConnectionField(RoomType)
    all_renter_rooms = DjangoFilterConnectionField(
        RoomType, orderBy=graphene.String(), status=graphene.String()
    )
    ...
    def resolve_all_Buildings(root, info, **kwargs):
        queryset = (
            ...
            if kwargs.get("globalSearch")
            else Building.objects.filter(owner=info.context.user)
        )
        ...
        if "address" in kwargs.get("orderBy"):
            ...
            return queryset.annotate(total_renter=Count("rooms__renter")).order_by(
                kwargs.get("orderBy")
            )
        elif "total_rooms" in kwargs.get("orderBy"):
            return queryset.annotate(total_rooms=Count("rooms__id")).order_by(
                kwargs.get("orderBy")
            )
    ...
    def resolve_all_properties_for_renter(root, info, **kwargs):
        ...
        if "address" in kwargs.get("orderBy"):
            ...
            return (
                Building.objects.filter(rooms__renter=info.context.user)
    ...
    def resolve_all_renter_rooms(root, info, **kwargs):
        ...
        if status:
            queryset = (
                queryset.filter(payments__isnull=True)
                if "No_payment_Yet" in status
                else queryset.filter(payments__status=status)
            )
        ...
        if orderBy:
            if "roomNo" in orderBy:
                ...
                order_field = (
                    ...
                    if "-" not in orderBy
                    else "-building__owner__username"
                )
    ...
    def resolve_all_available_rooms(root, info, **kwargs):
        return (
            Room.objects.filter(building__owner=info.context.user)
            .exclude(renter__isnull=False)
            .order_by("-id")
        )
