...
class RenterType(DjangoObjectType):
    ...
    def resolve_building_name(parent, info, **kwargs):
        building_name = parent.renter.values_list(
            "building__name", flat=True
        ).distinct()
        return building_name

    def resolve_rent_amount(parent, info, **kwargs):
        rent_amount = parent.renter.annotate(
...
class UserType(DjangoObjectType):
    ...
    def resolve_total_rent_amount(parent, info, **kwargs):
        ...
        total_rent_amount = (
            Room.objects.filter(renter=parent, building__owner=user)
            .annotate(
                rent_amount_numeric=Cast(
                    "rent_amount", models.DecimalField(max_digits=10, decimal_places=2)
                )
    ...
    def resolve_rented_rooms(parent, info, **kwargs):
        return Room.objects.filter(
            renter=parent, building__owner=info.context.user
        ).order_by("-id")
